...
class Tello:
    ...
    def go_xyz_speed(self, x: int, y: int, z: int, speed: int):
        ...
        """
        x = self.set_limit(x, 20, 500)
        y = self.set_limit(y, 20, 500)
        z = self.set_limit(z, 20, 500)
    ...
    def curve_xyz_speed(self, x1: int, y1: int, z1: int, x2: int, y2: int, z2: int, speed: int):
        ...
        Arguments:
            ...
            speed: 10-60
        """
        x1 = self.set_limit(x1, -500, 500)
        y1 = self.set_limit(y1, -500, 500)
        z1 = self.set_limit(z1, -500, 500)
        x2 = self.set_limit(x2, -500, 500)
        y2 = self.set_limit(y2, -500, 500)
        z2 = self.set_limit(z2, -500, 500)
    ...
    def go_xyz_speed_yaw_mid(self, x: int, y: int, z: int, speed: int, yaw: int, mid1: int, mid2: int):
        ...
        z = self.set_limit(z, 20, 500)
        speed = self.set_limit(speed, 10, 100)
        yaw = self.set_limit(yaw, -360, 360)
        mid1 = self.set_limit(mid1, 1, 8)
        mid2 = self.set_limit(mid2, 1, 8)
